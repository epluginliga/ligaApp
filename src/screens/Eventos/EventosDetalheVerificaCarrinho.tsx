...
function verificaSeCarrinhoTemUtilizador(
   ...
   const Temutilizadores = ingresso
      .filter(usuario => usuario.dados_atribuir.length > 0)
...
export function ButtonComprarIngressos({
   evento,
   ...
   return (
      <>
         <ModalSmall minHeight="25%" maxHeight={250} ativo={mostraModal}>
            {carrinho && (
               <VStack gap="xl">
                     ...
                     >
                        ...
                        <Pressable
                           onPress={() => {
                              ...
                              if (
                                 carrinho.status === 'aguardando_pagamento_pix'
                              ) {
                                 return navigate('CheckoutPix');
                              }

                              const ingresso = carrinho.eventos.flatMap(
                                 ingresso => ingresso.ingressos,
                              );

                              atualizaIngressoAoEvento(ingresso);
                              const rota =
                                 verificaSeCarrinhoTemUtilizador(ingresso);
