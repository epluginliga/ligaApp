...
import {
   ModalSmallButton,
   ModalSmallButtonAction,
} from '../../components/Modal/ModalSmall';
...
function BotaoTransferir({ item, ...rest }: BotaoTransferirProps) {
   const dataLimite = format(
      item.evento_data_limite_transferencia,
...
export function IngressosDisponivel() {
         ...
         <>
            ...
            >
               <VStack>
                  ...
                  <Section.SubTitle>
                     Deseja realemente devolver o ingresso?
                  </Section.SubTitle>
               </VStack>
               ...
               >
                  <Pressable
                     onPress={() =>
                        handleDevolverIngresso.mutate(item.bilhete_id)
                     }
                  >
                     <HStack
                        alignItems="center"
                     ...
                     >
                        ...
                        />
                        <Text variant="botaoLink" color="primary">
                           Devolver
                        </Text>
                     </HStack>
                  </Pressable>
                  ...
                  <Pressable
                     onPress={() => modalDevolverPedido.current?.close()}
                  >
                     <HStack
                        alignItems="center"
                        backgroundColor="bege"
                     ...
                     >
                        ...
                        />
                        <Text variant="botaoLink" color="greenDark">
                           Voltar
                        </Text>
                     </HStack>
                  </Pressable>
            ...
            <VStack flex={1.6} justifyContent="space-around">
               ...
               <VStack gap="sm">
                  ...
                  <Card.Span leftIcon={<Icon.Pin size={16} />}>
                     {item.evento_cidade} | {item.evento_estado} -{' '}
                     {dataEvento.hora() || 'hora não definida'}
                  </Card.Span>
                  ...
                  {usuarioRecebedor && (
                     <Card.Span leftIcon={<Icon.ArrowPath />}>
                        ...
                        <Card.Span color="azul" fontWeight="500">
                           {item.nome_compra}
                        </Card.Span>
                     </Card.Span>
                  )}
               ...
               >
                  <BotaoTransferir
                     item={item}
                     onPress={() =>
                        navigate.navigate('IngressoTranserir', {
                           ingresso_id: item.bilhete_id,
                        })
                     }
                  />
                  <BotaoDevolver id={item.bilhete_id} item={item} />
   ...
   const ingressosDisponiveis = data.data?.filter(
      item => user.documento === item.cpf_compra,
   );
